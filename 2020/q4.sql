/*
Advent of Code 2020 question 4 solution using sqlite3
...
WHERE element <> ''
AND INSTR(element, "byr") > 0
AND INSTR(element, "iyr") > 0
AND INSTR(element, "eyr") > 0
AND INSTR(element, "hgt") > 0
AND INSTR(element, "hcl") > 0
AND INSTR(element, "ecl") > 0
AND INSTR(element, "pid") > 0;
...
actual_values (byr, iyr, eyr, hgt, hcl, ecl, pid) AS (
SELECT
CAST(
    SUBSTR(
        SUBSTR(element, INSTR(element, "byr")),
        5,
        CASE
            WHEN INSTR(SUBSTR(element, INSTR(element, "byr")), ' ') > 0 THEN
                INSTR(SUBSTR(element, INSTR(element, "byr")), ' ') -5
            ELSE
                LENGTH(SUBSTR(element, INSTR(element, "byr")))
        END
    ) AS INTEGER
) AS byr,
CAST(
    SUBSTR(
        SUBSTR(element, INSTR(element, "iyr")),
        5,
        CASE
            WHEN INSTR(SUBSTR(element, INSTR(element, "iyr")), ' ') > 0 THEN
                INSTR(SUBSTR(element, INSTR(element, "iyr")), ' ') -5
            ELSE
                LENGTH(SUBSTR(element, INSTR(element, "iyr")))
        END
    ) AS INTEGER
) AS iyr,
CAST(
    SUBSTR(
        SUBSTR(element, INSTR(element, "eyr")),
        5,
        CASE
            WHEN INSTR(SUBSTR(element, INSTR(element, "eyr")), ' ') > 0 THEN
                INSTR(SUBSTR(element, INSTR(element, "eyr")), ' ') -5
            ELSE
                LENGTH(SUBSTR(element, INSTR(element, "eyr")))
        END
    ) AS INTEGER
) AS eyr,
SUBSTR(
    SUBSTR(element, INSTR(element, "hgt")),
    5,
    CASE
        WHEN INSTR(SUBSTR(element, INSTR(element, "hgt")), ' ') > 0 THEN
            INSTR(SUBSTR(element, INSTR(element, "hgt")), ' ') -5
        ELSE
            LENGTH(SUBSTR(element, INSTR(element, "hgt")))
    END
) AS hgt,
SUBSTR(
    SUBSTR(element, INSTR(element, "hcl")),
    5,
    CASE
        WHEN INSTR(SUBSTR(element, INSTR(element, "hcl")), ' ') > 0 THEN
            INSTR(SUBSTR(element, INSTR(element, "hcl")), ' ') -5
        ELSE
            LENGTH(SUBSTR(element, INSTR(element, "hcl")))
    END
) AS hcl,
SUBSTR(
    SUBSTR(element, INSTR(element, "ecl")),
    5,
    CASE
        WHEN INSTR(SUBSTR(element, INSTR(element, "ecl")), ' ') > 0 THEN
            INSTR(SUBSTR(element, INSTR(element, "ecl")), ' ') -5
        ELSE
            LENGTH(SUBSTR(element, INSTR(element, "ecl")))
    END
) AS ecl,
SUBSTR(
    SUBSTR(element, INSTR(element, "pid")),
    5,
    CASE
        WHEN INSTR(SUBSTR(element, INSTR(element, "pid")), ' ') > 0 THEN
            INSTR(SUBSTR(element, INSTR(element, "pid")), ' ') -5
        ELSE
            LENGTH(SUBSTR(element, INSTR(element, "pid")))
    END
) AS pid
FROM split
WHERE element <> ''
AND INSTR(element, "byr") > 0
AND INSTR(element, "iyr") > 0 AND CAST(iyr AS INTEGER)
AND INSTR(element, "eyr") > 0 AND CAST(eyr AS INTEGER)
AND INSTR(element, "hgt") > 0
AND INSTR(element, "hcl") > 0
AND INSTR(element, "ecl") > 0
AND INSTR(element, "pid") > 0
...
SELECT COUNT(*) FROM actual_values
WHERE byr BETWEEN 1920 AND 2002
AND iyr BETWEEN 2010 AND 2020
AND eyr BETWEEN 2020 AND 2030
AND (
    (INSTR(hgt, "in") > 0 AND CAST(SUBSTR(hgt, 1, LENGTH(hgt)-2) AS INT) BETWEEN 59 AND 76)
    OR
    (INSTR(hgt, "cm") > 0 AND CAST(SUBSTR(hgt, 1, LENGTH(hgt)-2) AS INT) BETWEEN 150 AND 193)
)
AND SUBSTR(hcl, 1, 1) = '#' AND LENGTH(hcl) == 7 AND NOT substr(hcl, 2, LENGTH(hcl) - 1) GLOB '*[^0-9a-f]*'
AND ecl IN ('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth')
AND NOT pid GLOB '*[^0-9]*' AND LENGTH(pid) = 9;
