	...
	#for iters in range(10000):
		for machine_index in range(len(inputs)):
			if runnable[machine_index]:
				if len(inp[machine_index]) == 0 and data[machine_index][op_code_indexes[machine_index]] % 100 == 3:
					continue
				offset, value = read_instruction(op_code_indexes[machine_index], data[machine_index], inp[machine_index])
			...
			for input4 in range(5):
				...
				for input5 in range(5):
					...
					if result5 > max_thrust:
						max_thrust = result5
					debug_print(str(input1) + ' ' + str(input2) + ' ' +  str(input3) + ' ' +  str(input4) + ' ' +  str(input5) + ' ' +  str(result5))
